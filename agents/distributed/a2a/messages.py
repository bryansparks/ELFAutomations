...
class A2AMessage(BaseModel):
    ...
    conversation_id: Optional[str] = Field(
        default=None, description="ID for grouping related messages"
    )
    correlation_id: Optional[str] = Field(
        default=None, description="ID for request/response correlation"
    )
...
class TaskRequestMessage(A2AMessage):
    ...
    message_type: Literal[MessageType.TASK_REQUEST] = Field(
        default=MessageType.TASK_REQUEST
    )
    ...
    def __init__(self, **data):
        ...
        if "task_description" not in self.content:
            raise ValueError(
                "TaskRequestMessage requires 'task_description' in content"
            )
...
class TaskResponseMessage(A2AMessage):
    ...
    message_type: Literal[MessageType.TASK_RESPONSE] = Field(
        default=MessageType.TASK_RESPONSE
    )
...
class CollaborationRequestMessage(A2AMessage):
    ...
    message_type: Literal[MessageType.COLLABORATION_REQUEST] = Field(
        default=MessageType.COLLABORATION_REQUEST
    )
    ...
    def __init__(self, **data):
        ...
        for field in required_fields:
            if field not in self.content:
                raise ValueError(
                    f"CollaborationRequestMessage requires '{field}' in content"
                )
...
class StatusUpdateMessage(A2AMessage):
    ...
    message_type: Literal[MessageType.STATUS_UPDATE] = Field(
        default=MessageType.STATUS_UPDATE
    )
...
class CapabilityQueryMessage(A2AMessage):
    ...
    message_type: Literal[MessageType.CAPABILITY_QUERY] = Field(
        default=MessageType.CAPABILITY_QUERY
    )
...
class CapabilityResponseMessage(A2AMessage):
    ...
    message_type: Literal[MessageType.CAPABILITY_RESPONSE] = Field(
        default=MessageType.CAPABILITY_RESPONSE
    )
    ...
    def __init__(self, **data):
        ...
        if "capabilities" not in self.content:
            raise ValueError(
                "CapabilityResponseMessage requires 'capabilities' in content"
            )
...
def create_task_response(
    ...
    **kwargs,
    ...
    content = {"status": status, "completed_at": datetime.utcnow().isoformat()}

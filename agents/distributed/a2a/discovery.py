...
class DiscoveryService:
    ...
    def __init__(self, discovery_endpoint: Optional[str] = None):
        ...
        if discovery_endpoint:
            self.logger.info(f"Discovery service configured with endpoint: {discovery_endpoint}")
        else:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                response = await client.post(
                    f"{self.discovery_endpoint}/agents",
                    json=payload
                )
                
                if response.status_code in [200, 201]:
                    self.logger.info(f"Agent {agent_card.agent_id} registered with central discovery")
                    return True
                else:
                    self.logger.error(f"Central discovery registration failed: {response.status_code}")
                    return False
    ...
    async def _unregister_from_central_service(self, agent_id: str) -> bool:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                response = await client.delete(f"{self.discovery_endpoint}/agents/{agent_id}")
                
                if response.status_code in [200, 204, 404]:
                    self.logger.info(f"Agent {agent_id} unregistered from central discovery")
                    return True
                else:
                    self.logger.error(f"Central discovery unregistration failed: {response.status_code}")
                    return False
    ...
    async def _get_agent_from_central_service(self, agent_id: str) -> Optional[AgentCard]:
        """Get agent information from central discovery service."""
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                response = await client.get(f"{self.discovery_endpoint}/agents/{agent_id}")
                ...
                if response.status_code == 200:
                    ...
                    self.logger.error(f"Failed to get agent from central discovery: {response.status_code}")
                    return None
    ...
    async def _discover_from_central_service(self, capabilities: Optional[List[str]] = None) -> List[AgentCard]:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                response = await client.get(f"{self.discovery_endpoint}/agents", params=params)
                ...
                if response.status_code == 200:
                    ...
                    self.logger.error(f"Failed to discover agents from central service: {response.status_code}")
                    return []
    ...
    async def _discover_locally(self, capabilities: Optional[List[str]] = None) -> List[AgentCard]:

...
class A2AClientManager:
    ...
    def __init__(
        ...
        # Discovery service
        self.discovery = DiscoveryService(discovery_endpoint) if discovery_endpoint else None
    ...
    async def send_message(self, target_agent_id: str, message: A2AMessage) -> Dict[str, Any]:
        """Send a message to another agent."""
        try:
            self.logger.info(f"Sending {message.message_type.value} message to {target_agent_id}")
            
            # Get or create client for target agent
            client = await self._get_client(target_agent_id)
            
            if not client:
                raise Exception(f"Could not establish connection to agent: {target_agent_id}")
    ...
    async def broadcast_message(self, message: A2AMessage, target_agents: List[str]) -> Dict[str, Any]:
        ...
        for target_agent_id in target_agents:
            try:
                ...
                self.logger.error(f"Failed to send broadcast message to {target_agent_id}: {e}")
    ...
    async def _get_client(self, target_agent_id: str) -> Optional[A2AClient]:
        ...
        try:
            ...
            if target_agent_id in self.clients:
                ...
                if last_used and (datetime.utcnow() - last_used).seconds < self.client_cache_ttl:
            ...
            if self.discovery:
                ...
                if not agent_card:
                    self.logger.error(f"Agent {target_agent_id} not found in discovery service")
                    return None
                
                # Create new client
                endpoint = agent_card.endpoints.get("a2a")
                if not endpoint:
                    self.logger.error(f"No A2A endpoint found for agent {target_agent_id}")
                    return None
                ...
                self.logger.warning(f"No discovery service available for agent {target_agent_id}")
                return None
    ...
    async def _simulate_a2a_call(self, client: A2AClient, payload: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _record_message(self, message: A2AMessage, response: Dict[str, Any], status: str) -> None:

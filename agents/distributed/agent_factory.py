...
class AgentFactory:
    ...
    def _generate_k8s_service(self, spec: AgentSpec) -> str:
        ...
        service = {
            ...
            "metadata": {
                ...
                "namespace": "elf-automations",
                "labels": {"app": spec.agent_id, "department": spec.department},
            },
            "spec": {
                "selector": {"app": spec.agent_id},
                "ports": [
                    {
                        "name": "a2a",
                        "port": spec.a2a_port,
    ...
    def _generate_service_monitor(self, spec: AgentSpec) -> str:
        ...
        monitor = {
            ...
            "spec": {
                "selector": {"matchLabels": {"app": spec.agent_id}},
                "endpoints": [
    ...
    def _generate_network_policy(self, spec: AgentSpec) -> str:
        ...
        policy = {
            ...
            "spec": {
                "podSelector": {"matchLabels": {"app": spec.agent_id}},
                ...
                "ingress": [
                    {
                        "from": [
                            {
                                "namespaceSelector": {
                                    "matchLabels": {"name": "elf-automations"}
                                }
                            }
                        ],
                        "ports": [
                            {"protocol": "TCP", "port": spec.a2a_port},
                            {"protocol": "TCP", "port": spec.health_port},
                        ],
                    }
                ],
            },
        ...
        if agent_llm:
            env_vars.extend(
                [
                    {
                        "name": "LLM_PROVIDER",
                        "value": agent_llm.get("provider", "openai"),
                    },
                    {
                        "name": "LLM_MODEL",
                        "value": agent_llm.get("model", "gpt-4o-mini"),
                    },
                    {
                        "name": "LLM_TEMPERATURE",
                        "value": str(agent_llm.get("temperature", 0.3)),
                    },
                    {
                        "name": "LLM_MAX_TOKENS",
                        "value": str(agent_llm.get("max_tokens", 1500)),
                    },
                ]
            )
    ...
    def save_infrastructure(self, agent_specs: List[AgentSpec], crew_name: str):
        ...
        print(
            f"✅ Saved infrastructure for {len(agent_specs)} agents in crew '{crew_name}'"
        )
...
if __name__ == "__main__":
    ...
    sales_manager_spec = AgentSpec(
        ...
        scaling={"replicas": 1, "minReplicas": 1, "maxReplicas": 2, "targetCPU": 70},
        llm_config={
            "model": "gpt-3.5-turbo",
            "api_key": "YOUR_API_KEY",
            "api_secret": "YOUR_API_SECRET",
        },
        api_keys={"api_key": "YOUR_API_KEY", "api_secret": "YOUR_API_SECRET"},
    )

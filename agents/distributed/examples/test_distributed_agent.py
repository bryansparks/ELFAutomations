...
from ..a2a.messages import create_task_request, MessagePriority
...
async def test_agent_health(agent: SalesAgent):
    ...
    assert health_status["status"] in ["healthy", "starting"], f"Unexpected health status: {health_status}"
...
async def test_task_execution(agent: SalesAgent):
    ...
    task_description = "Analyze the sales potential for a new SaaS product in the healthcare industry"
    context = {
        "industry": "healthcare",
        "product_type": "SaaS",
...
async def test_lead_qualification(agent: SalesAgent):
    ...
    assert response["status"] == "task_completed", f"Lead qualification failed: {response}"
    
    # Check sales metrics updated
    metrics = agent.get_sales_metrics()
    assert metrics["sales_metrics"]["leads_qualified"] > 0, "Lead qualification count not updated"

...
class HealthCheckMixin:
    ...
    async def _perform_health_check(self) -> None:
        ...
        try:
            ...
            if failed_checks:
                self.logger.warning(f"Health check failed: {failed_checks}, status: {self.health_status.value}")
            else:
    ...
    async def _check_message_handling(self) -> bool:
        ...
        try:
            ...
            return hasattr(self, 'handle_message') and callable(getattr(self, 'handle_message'))
        except Exception:
            return False

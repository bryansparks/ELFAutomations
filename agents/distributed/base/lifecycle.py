...
class AgentLifecycleMixin:
        ...
        try:
            ...
            try:
                await asyncio.wait_for(
                    self._wait_for_shutdown_complete(),
                    timeout=self.shutdown_timeout
                )
            except asyncio.TimeoutError:
                self.logger.warning(f"Graceful shutdown timeout ({self.shutdown_timeout}s), forcing exit")
    ...
    def remove_lifecycle_callback(self, state: LifecycleState, callback: Callable) -> None:

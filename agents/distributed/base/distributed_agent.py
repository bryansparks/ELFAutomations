...
class AgentConfig(BaseModel):
    ...
    discovery_endpoint: Optional[str] = Field(default=None, description="A2A discovery service endpoint")
    
    # Kubernetes Configuration
    namespace: str = Field(default="elf-automations", description="Kubernetes namespace")
    service_name: Optional[str] = Field(default=None, description="Kubernetes service name")
...
class DistributedCrewAIAgent(HealthCheckMixin, AgentLifecycleMixin):
    ...
    async def send_message(self, target_agent_id: str, message: A2AMessage) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(f"Sending message to {target_agent_id}: {message.message_type}")
            
            # Send via A2A client manager
            response = await self.a2a_client_manager.send_message(target_agent_id, message)
    ...
    def get_agent_card(self) -> AgentCard:
        ...
        return AgentCard(
            ...
            description=f"{self.config.backstory[:200]}..." if len(self.config.backstory) > 200 else self.config.backstory,
            version="1.0.0",
    ...
    async def _start_a2a_server(self) -> None:
        ...
        try:
            self.logger.info(f"Starting A2A server on {self.config.a2a_host}:{self.config.a2a_port}")

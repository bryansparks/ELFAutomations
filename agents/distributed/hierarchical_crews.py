...
from .a2a.mock_a2a import AgentCard, AgentCapabilities, AgentSkill
...
class HierarchicalCrewManager:
    ...
    def create_agent(self, agent_id: str, capabilities: List[str] = None, tools: List[Any] = None) -> HierarchicalAgent:
    ...
    def create_executive_crew(self) -> Crew:
        ...
        required_agents = ["chief-ai-agent", "sales-manager-agent", "marketing-manager-agent"]
        ...
        executive_crew = Crew(
            agents=[
                ...
                self.agents["sales-manager-agent"].get_participant_agent(),  # Participant role
                self.agents["marketing-manager-agent"].get_participant_agent(),  # Participant role
            ],
    ...
    def create_sales_crew(self) -> Crew:
        ...
        # Ensure required agents exist
        required_agents = ["sales-manager-agent", "sdr-agent", "sales-representative-agent"]
        for agent_id in required_agents:
            if agent_id not in self.agents:
                self.create_agent(agent_id)
        ...
        sales_crew = Crew(
            agents=[
                ...
                self.agents["sales-representative-agent"].get_participant_agent(),  # Participant role
            ],
            tasks=[],  # Tasks will be added dynamically
            process=Process.hierarchical,
            manager_llm=self.manager_llm,
    ...
    def create_marketing_crew(self) -> Crew:
        ...
        # Ensure required agents exist
        required_agents = ["marketing-manager-agent", "content-creator-agent", "social-media-agent"]
        for agent_id in required_agents:
            if agent_id not in self.agents:
                self.create_agent(agent_id)
        ...
        marketing_crew = Crew(
            agents=[
                self.agents["marketing-manager-agent"].get_manager_agent(),  # Manager role
                self.agents["content-creator-agent"].get_participant_agent(),  # Participant role
                self.agents["social-media-agent"].get_participant_agent(),  # Participant role
            ],
            tasks=[],  # Tasks will be added dynamically
            process=Process.hierarchical,
            manager_llm=self.manager_llm,
...
async def demo_hierarchical_crews():
    ...
    exec_result = await crew_manager.execute_hierarchical_task("executive-crew", strategic_task)
    ...
    sales_result = await crew_manager.execute_hierarchical_task("sales-crew", sales_task)
    ...
    marketing_result = await crew_manager.execute_hierarchical_task("marketing-crew", marketing_task)

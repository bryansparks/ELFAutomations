...
class LangGraphBaseAgent(ABC):
    ...
    def __init__(
        ...
        self.logger = structlog.get_logger().bind(
            agent_id=agent_id, agent_name=name, department=department
        )

        # Initialize AgentGateway client for MCP access
        self.gateway_client = AgentGatewayClient(
            agent_id=agent_id, gateway_url=gateway_url, api_key=gateway_api_key
        )

        # Initialize LLM
        self.llm = ChatAnthropic(
            model="claude-3-5-sonnet-20241022", temperature=0.1, max_tokens=4000
        )
    ...
    async def _process_task_node(
        self, state: LangGraphAgentState
    ) -> LangGraphAgentState:
    ...
    def _should_use_tools(self, state: LangGraphAgentState) -> str:
        ...
        if messages:
            ...
            if isinstance(last_message, AIMessage):
                ...
                if any(
                    keyword in content
                    for keyword in ["tool", "search", "database", "api", "call"]
                ):
                    return "use_tools"
    ...
    async def process_message(
        ...
        try:
            ...
            self.logger.error(
                "Message processing failed", error=str(e), thread_id=thread_id
            )
            self.error_count += 1
            raise

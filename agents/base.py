...
class AgentConfig(BaseModel):
    ...
    max_tokens: int = Field(default=4000, ge=100, le=8000, description="Maximum tokens per response")
    ...
    department_access: List[str] = Field(default_factory=list, description="Departments this agent can access")
    escalation_threshold: int = Field(default=3, description="Number of failures before escalation")
...
class AgentMessage(BaseModel):
    ...
    id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Message unique identifier")
    from_agent: str = Field(description="Sender agent ID")
    to_agent: str = Field(description="Recipient agent ID")
    message_type: str = Field(description="Type of message")
    content: Dict[str, Any] = Field(description="Message content")
    priority: int = Field(default=3, ge=1, le=5, description="Message priority (1=highest)")
    timestamp: datetime = Field(default_factory=datetime.utcnow)
    requires_response: bool = Field(default=False)
    correlation_id: Optional[str] = Field(default=None, description="For tracking related messages")
...
Always respond in a structured format and provide clear reasoning for your decisions."""
    ...
    async def send_message(self, to_agent: str, message_type: str, content: Dict[str, Any], 
                          priority: int = 3, requires_response: bool = False) -> str:
    ...
    async def _deliver_message(self, message: AgentMessage) -> None:
        ...
        if target_agent:
            ...
            self.logger.error("Target agent not found", target_agent_id=message.to_agent)

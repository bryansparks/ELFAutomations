...
"""
    ...
    async def validate_infrastructure(self) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_results["kagent_lifecycle"] = health_check.status.value in [
                "running",
                "created",
            ]
            ...
            validation_results["overall_status"] = (
                "passed" if all_tests_passed else "failed"
            )

            self.logger.info(
                "Infrastructure validation completed", results=validation_results
            )
...
def create_demo_agent(
    ...
    return DemoAgent(
        agent_id=agent_id, gateway_url=gateway_url, gateway_api_key=gateway_api_key
    )
...
async def main():
    ...
    try:
        ...
        print(
            f"Workflow result: {len(workflow_result.get('messages', []))} messages processed"
        )
        ...
        print(
            f"MCP test result: {len(mcp_result.get('available_tools', []))} tools available"
        )

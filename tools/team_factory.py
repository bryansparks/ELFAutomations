...
class TeamMember:
    ...
    skills: List[str]
    system_prompt: str
    communicates_with: List[str] = field(default_factory=list)  # Intra-team communication
    manages_teams: List[str] = field(default_factory=list)  # Inter-team A2A communication
...
class TeamFactory:
    ...
    def __init__(self):
        ...
        self.EXECUTIVE_TEAM_MAPPINGS = {
            "Chief Executive Officer": ["executive-team"],  # CEO manages all through other executives
            ...
            "Chief Operations Officer": ["operations-team", "hr-team", "facilities-team"],
            "Chief Financial Officer": ["finance-team", "accounting-team", "budget-team"],
    ...
    def create_team(self):
        ...
        team_spec = self._analyze_team_requirements(description, framework, llm_provider, llm_model)
        ...
        if reports_to and team_spec.members:
            ...
            if reports_to in self.EXECUTIVE_TEAM_MAPPINGS:
                ...
                manager.manages_teams = []  # Will be configured based on user input later
        ...
        if self._register_team_in_registry(team_spec):
            ...
            self.console.print("[dim]The team files were generated but may need manual registry update[/dim]")
        
    def _choose_framework(self) -> str:
        """Let user choose the agentic framework"""
        self.console.print("[bold]Which agentic framework would you like to use?[/bold]\n")
    ...
    def _get_team_description(self) -> str:
        ...
        self.console.print("[dim]Example: I need a marketing team that can create content, manage social media, and run campaigns. They should be able to analyze performance and adjust strategies.[/dim]\n")
    ...
    def _get_organizational_placement(self) -> Tuple[str, Optional[str], Optional[str]]:
        ...
        self.console.print("[dim]Use dot notation: department.subteam (e.g., marketing.socialmedia, engineering.backend, executive)[/dim]")
        ...
        self.console.print("[dim]  - marketing.socialmedia (sub-team under marketing)[/dim]")
        self.console.print("[dim]  - engineering.platform.infrastructure (nested sub-teams)[/dim]\n")
    ...
    def _analyze_team_requirements(self, description: str, framework: str, llm_provider: str, llm_model: str) -> TeamSpecification:
        ...
        suggested_team = self._generate_team_suggestion(description, framework, llm_provider, llm_model)
        
        return suggested_team
    
    def _generate_team_suggestion(self, description: str, framework: str, llm_provider: str = "OpenAI", llm_model: str = "gpt-4") -> TeamSpecification:
        ...
        if department == "marketing":
            members = [
                TeamMember(
                    ...
                    skills=["Strategic planning", "Team leadership", "Analytics", "Campaign management"],
                    ...
                    communicates_with=["Content Creator", "Social Media Manager", "Analytics Expert"],
                    manages_teams=["social-media-team", "content-team"]  # Can delegate to subordinate teams
                ),
                TeamMember(
                    ...
                    skills=["Writing", "SEO", "Content strategy", "Web content management"],
                ...
                TeamMember(
                    ...
                    communicates_with=["Marketing Manager", "Content Creator", "Analytics Expert"]
                ),
                TeamMember(
            ...
            members = [
                TeamMember(
                    ...
                    communicates_with=["Chief Technology Officer", "Chief Marketing Officer", "Chief Operations Officer", "Chief Financial Officer"],
                    manages_teams=self.EXECUTIVE_TEAM_MAPPINGS.get("Chief Executive Officer", [])
                ),
                TeamMember(
                    ...
                    communicates_with=["Chief Executive Officer", "Chief Operations Officer", "Chief Marketing Officer"],
                    manages_teams=self.EXECUTIVE_TEAM_MAPPINGS.get("Chief Technology Officer", [])
                ),
                TeamMember(
                    ...
                    communicates_with=["Chief Executive Officer", "Chief Technology Officer", "Chief Financial Officer"],
                    manages_teams=self.EXECUTIVE_TEAM_MAPPINGS.get("Chief Marketing Officer", [])
                ),
                TeamMember(
                    ...
                    communicates_with=["Chief Executive Officer", "Chief Technology Officer", "Chief Financial Officer"],
                    manages_teams=self.EXECUTIVE_TEAM_MAPPINGS.get("Chief Operations Officer", [])
                ),
                TeamMember(
                    ...
                    communicates_with=["Chief Executive Officer", "Chief Marketing Officer", "Chief Operations Officer"],
                    manages_teams=self.EXECUTIVE_TEAM_MAPPINGS.get("Chief Financial Officer", [])
                )
            ]
        else:
            # Default small team
            members = [
                TeamMember(
                    role="Team Lead",
                    responsibilities=["Coordinate team", "Set priorities", "Communicate with other teams"],
                ...
                TeamMember(
                    role="Specialist 1",
                    responsibilities=["Execute core tasks", "Collaborate with team", "Report progress"],
                ...
                TeamMember(
                    role="Specialist 2",
                    responsibilities=["Support team objectives", "Provide expertise", "Complete assignments"],
    ...
    def _review_and_refine_team(self, team_spec: TeamSpecification) -> TeamSpecification:
        ...
        if Confirm.ask("\nWould you like to proceed with this team composition?"):
            ...
            self.console.print("[yellow]Team refinement not yet implemented. Proceeding with suggested composition.[/yellow]")
            return team_spec
...
'''.format(team_name=team_spec.name)
        ...
        for member in team_spec.members:
            ...
            if team_spec.llm_provider == "OpenAI":
                ...
                llm_init = f'ChatAnthropic(model="{team_spec.llm_model}", temperature=0.7)'
...
{chr(10).join(f"    - {r}" for r in member.responsibilities)}
    ...
    def __init__(self, tools: Optional[List] = None):
        ...
        self.agent = self._create_agent(){a2a_init}
...
'''
        ...
        init_content += "\n".join(agent_imports)
        init_content += "\n\n__all__ = ["
        init_content += ", ".join(f'"{member.role.replace(" ", "") + "Agent"}"' for member in team_spec.members)
        init_content += "]\n"
...
'''
    ...
    def _create_crew(self) -> Crew:
        ...
        return Crew(
            agents=crew_agents,
            process=Process.''' + ("hierarchical" if len(team_spec.members) > 4 else "sequential") + ''',
            verbose=True,
    ...
    def get_team_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": "active",
            "communication_pattern": ''' + str(team_spec.communication_pattern) + '''
        }
...
def get_orchestrator(tools: Optional[Dict[str, List]] = None):
    ...
    if _orchestrator_instance is None:
        _orchestrator_instance = ''' + team_spec.name.replace("-", " ").title().replace(" ", "") + '''Crew(tools=tools)
...
{chr(10).join(f"    - {r}" for r in member.responsibilities)}
    ...
    def _initialize_custom_workflow(self):
        ...
        workflow.add_node("delegate_tasks", self._delegate_tasks_node)""" if is_manager else "") + '''
        ...
        workflow.add_edge("delegate_tasks", "review_results")""" if is_manager else """
    ...
    async def _analyze_task_node(self, state: {class_name}State) -> {class_name}State:
        ...
        4. Should this be delegated to subordinate teams?""" if is_manager else "") + '''
        """)
    ...
    async def _delegate_tasks_node(self, state: {class_name}State) -> {class_name}State:
        ...
        state["messages"] = messages
        return state{a2a_methods}""" if is_manager else "") + f'''
...
'''
    ...
    def _generate_langgraph_workflow(self, team_spec: TeamSpecification, team_dir: Path):
...
'''
        ...
        for member in team_spec.members:
            ...
            workflow_content += f'        workflow.add_node("{node_name}", self._{node_name}_node)\n'
...
'''
        ...
        for member in team_spec.members:
            node_name = member.role.lower().replace(" ", "_") + "_work"
            workflow_content += f'        workflow.add_edge("{node_name}", "coordinate_work")\n'
...
'''
    ...
    def _generate_langgraph_deployable_script(self, team_spec: TeamSpecification, team_dir: Path):
...
'''
        ...
        for member in team_spec.members:
            node_name = member.role.lower().replace(" ", "_") + "_work"
            readme_content += f"4. **{member.role} Work**: Executes {member.role}-specific tasks\n"
      ...
      - name: team-runtime
        ...
        - name: OPENAI_API_KEY
          valueFrom:
            secretKeyRef:
              name: llm-api-keys
              key: openai-api-key""" if team_spec.llm_provider == "OpenAI" else f"""
...
"""
    ...
    def _generate_team_documentation(self, team_spec: TeamSpecification, team_dir: Path):
...
'''
            for manager in managers_with_teams:
                readme_content += f"**{manager.role}** manages the following teams via A2A:\n"
                for team in manager.manages_teams:
                    readme_content += f"- `{team}`\n"
                readme_content += "\n"
...
'''
    ...
    def _register_team_in_registry(self, team_spec: TeamSpecification) -> bool:
        ...
        if not REGISTRY_AVAILABLE:
            self.console.print("[yellow]Team Registry not available - skipping registration[/yellow]")
            return False
        ...
        try:
            ...
            for member in team_spec.members:
                if "Manager" in member.role or "Lead" in member.role:
                    ...
                    if "report to" in member.system_prompt and "via A2A" in member.system_prompt:
                        # Extract who they report to
                        import re
                        match = re.search(r'report to ([^.]+) via A2A', member.system_prompt)
                        if match:
            ...
            self.console.print(f"[green]✓ Registered {len(team_spec.members)} team members[/green]")
            ...
            if reports_to:
                if reports_to.startswith("Chief") and reports_to.endswith("Officer"):
                    ...
                    self.console.print(f"[green]✓ Registered reporting relationship to {reports_to}[/green]")
            ...
            for member_role, communicates_with in team_spec.communication_pattern.items():
                for target_role in communicates_with:
                    ...
                    try:
                        client.table('team_communication_patterns').insert(comm_data).execute()
                    except:
                        # Ignore duplicates
                        pass
            ...
            self.console.print("[green]✓ Team successfully registered in Team Registry[/green]")
            return True
...
"""
        ...
        self.console.print("[dim]   Apply this patch to update executive team configuration[/dim]")

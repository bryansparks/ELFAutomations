    ...
    def schedule_fetch_store():
        ...
        return {
            ...
            "connections": {
                ...
                "Fetch Data": {"main": [[{"node": "Store Data", "type": "main", "index": 0}]]}
            }
        }
...
"""
        ...
        # Fallback analysis
        return {
            "trigger_type": "webhook" if "webhook" in description.lower() else "schedule",
            "operations": ["process"],
            "data_sources": [],
            "data_destinations": []
        }
    ...
    async def deploy_workflow(self, 
                            ...
                            n8n_workflow_id: Optional[str] = None) -> str:
        ...
        if result.data:
            ...
            filename = f"{workflow['name'].lower().replace(' ', '_')}_{workflow_id}.json"
            ...
            print(f"3. Update registration: python scripts/n8n_workflow_manager.py update '{workflow['name']}' --workflow-id [ID]")
...
def main():
    ...
    parser.add_argument('--deploy', action='store_true', help='Deploy and register workflow')

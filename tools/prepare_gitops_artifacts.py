...
class GitOpsArtifactPreparer:
        ...
        for team_path in self.teams_dir.iterdir():
            if team_path.is_dir() and (team_path / "k8s").exists():
                ...
                if not self._check_docker_image(f"{team_name}"):
                    ...
                    print(f"       Run: cd {team_path} && python make-deployable-team.py")
                    print(f"       Then: docker build -t elf-automations/{team_name} .")
                    print(f"       And: docker push <your-registry>/elf-automations/{team_name}")
                    continue
        ...
        for mcp_path in self.mcps_dir.iterdir():
            if mcp_path.is_dir() and (mcp_path / "k8s").exists():
                ...
                if not self._check_docker_image(f"mcp-{mcp_name}"):
                    ...
                    print(f"       Then: docker build -t elf-automations/mcp-{mcp_name} .")
                    print(f"       And: docker push <your-registry>/elf-automations/mcp-{mcp_name}")
                    continue
    ...
    def _apply_patches(self, team_path: Path, team_name: str):
        ...
        if applied_patches:
            print(f"  ⚠️  Manual patch application needed for {len(applied_patches)} patches")
    ...
    def _generate_argocd_apps(self):
        ...
        teams_app = {
            ...
            "spec": {
                "project": "default",
                "source": {
                    "repoURL": os.getenv("GITOPS_REPO_URL", "https://github.com/your-org/gitops"),
                    "targetRevision": "HEAD",

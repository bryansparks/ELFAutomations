...
from ..models import TeamCharter, TeamMember, TeamSpecification
...
"""
        ...
        try:
            ...
            if json_match:
                ...
                self.logger.info(
                    f"Successfully analyzed team requirements: {len(result.get('roles', []))} roles identified"
                )
                return result
            else:
...
"""
    ...
    def _get_fallback_composition(
        self, charter: TeamCharter, description: str, framework: str
    ) -> Dict[str, Any]:
        """
    ...
    def _get_fallback_prompt(
        self, role: Dict[str, Any], charter: TeamCharter, pm_context: str
    ) -> str:
        """

...
def test_minio_connection():
    ...
    try:
        ...
        for tenant in test_tenants:
            ...
            if documents:
                ...
                for doc in documents[:3]:  # Show first 3
                    console.print(f"  - {doc.get('filename')} ({doc.get('size')} bytes)")
            else:
            ...
            if content:
                console.print(f"[green]✓ Document retrieved ({len(content)} bytes)[/green]")
                console.print(f"  Content preview: {content[:50].decode('utf-8', errors='ignore')}...")
            else:
        ...
        console.print("3. Default credentials: elfautomations / elfautomations2025secure")
        ...
        console.print("2. Check port forwarding: kubectl port-forward -n elf-storage svc/minio 30900:9000")

...
def verify_tables(supabase):
    ...
    tables_to_check = ["agent_evolutions", "ab_tests", "ab_test_results"]
...
def verify_views(supabase):
    ...
    views_to_check = [
        ...
        (
            "evolution_performance_summary",
            "SELECT * FROM evolution_performance_summary LIMIT 1",
        ),
...
def test_basic_operations(supabase):
    ...
    try:
        ...
        team_result = (
            supabase.table("teams").select("id").eq("name", "executive-team").execute()
        )

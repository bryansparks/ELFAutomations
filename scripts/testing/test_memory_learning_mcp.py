    ...
    try:
        ...
        retrieval_result = await client.call_tool(
            "retrieve_memories",
            {
                ...
                "filters": {"type": "learning", "min_importance": 0.7},
                "top_k": 5,
            },
        )
        ...
        patterns_result = await client.call_tool(
            ...
            {"task_type": "deployment", "min_confidence": 0.7, "limit": 5},
        )
        ...
        print(
            f"✅ Outcome analyzed: {outcome_result.get('analysis', {}).get('recommendation', 'No recommendation')}"
        )
        ...
        if patterns_result.get("patterns") and len(patterns_result["patterns"]) > 0:
            ...
            confidence_result = await client.call_tool(
                "update_pattern_confidence", {"pattern_id": pattern_id, "success": True}
            )
            print(
                f"✅ Pattern confidence updated: {confidence_result.get('new_confidence', 'Unknown')}"
            )
        else:
        ...
        insights_result = await client.call_tool(
            "get_team_insights", {"team_id": "devops-team-123", "timeframe": "month"}
        )
        print(
            f"✅ Team insights retrieved: {insights_result.get('insights', {}).get('statistics', {}).get('total_memories', 0)} memories"
        )
        ...
        collection_result = await client.call_tool(
            "create_collection",
            {
                ...
                "config": {"retention_days": 365, "auto_archive": True},
                "team_id": "devops-team-123",
            },
        )
        ...
        prune_result = await client.call_tool(
            "prune_old_memories",
            {"retention_policy": {"days": 30, "min_importance": 0.3}, "dry_run": True},
        )
        print(
            f"✅ Prune test: Would delete {prune_result.get('would_delete', 0)} memories"
        )
        ...
        export_result = await client.call_tool(
            ...
            {"format": "json", "filters": {"team_id": "devops-team-123"}},
        )
        print(
            f"✅ Knowledge exported: {export_result.get('statistics', {}).get('memories_exported', 0)} memories"
        )

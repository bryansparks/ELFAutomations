...
def test_schema_access():
    ...
    try:
        result = (
            client.from_("rag.tenants").select("*", count="exact").limit(1).execute()
        )
        console.print(
            f"[green]✓ Success! Found {result.count} rows in rag.tenants[/green]"
        )
        if result.data:
    ...
    try:
        ...
        console.print(
            f"[yellow]⚠ Unexpected success - table accessible without schema[/yellow]"
        )
    except Exception as e:

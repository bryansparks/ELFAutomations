...
def update_workflow(n8n_workflow_id: str):
    ...
    # Update the workflow
    result = supabase.table('n8n_workflows').update({
        'n8n_workflow_id': n8n_workflow_id,
        'webhook_url': f'http://n8n.n8n.svc.cluster.local:5678/webhook/{n8n_workflow_id}'
    }).eq('name', 'test-webhook-workflow').execute()
    
    if result.data:
        print(f"✓ Updated test workflow with n8n ID: {n8n_workflow_id}")
        print(f"  Webhook URL: http://n8n.n8n.svc.cluster.local:5678/webhook/{n8n_workflow_id}")
    else:

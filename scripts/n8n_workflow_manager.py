...
class WorkflowManager:
    ...
    def __init__(self):
        ...
        if not supabase_url or not supabase_key:
            raise ValueError("Supabase credentials not set. Set SUPABASE_URL and SUPABASE_KEY (or SUPABASE_ANON_KEY) in .env")
    ...
    def register_workflow(self, 
        ...
        try:
            result = self.supabase.table('n8n_workflows').insert(workflow_data).execute()
    ...
    def update_workflow(self, name: str, **kwargs):
        ...
        try:
            ...
            current = self.supabase.table('n8n_workflows').select('*').eq('name', name).single().execute()
            ...
            result = self.supabase.table('n8n_workflows').update(update_data).eq('name', name).execute()
    ...
    async def list_workflows(self, team: Optional[str] = None, category: Optional[str] = None):
...
async def main():
    ...
    register_parser.add_argument('--description', required=True, help='Workflow description')
    ...
    register_parser.add_argument('--trigger', required=True,
                               choices=['webhook', 'schedule', 'manual', 'event'])

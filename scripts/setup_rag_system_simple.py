...
def get_supabase_client() -> Client:
    ...
    if not url or not key:
        console.print("[red]Error: SUPABASE_URL and SUPABASE_KEY environment variables must be set![/red]")
        console.print("\nExample:")
...
def verify_schema(client: Client) -> bool:
    ...
    try:
        ...
        console.print(f"[green]✓ Schema verified! Found {result.count} tenants.[/green]")
...
def create_test_data(client: Client) -> bool:
    ...
    try:
        for tenant in test_tenants:
            ...
            existing = client.from_("rag.tenants").select("*").eq("name", tenant["name"]).execute()
            
            if not existing.data:
                result = client.from_("rag.tenants").insert(tenant).execute()
                console.print(f"[green]✓ Created tenant: {tenant['display_name']}[/green]")
            else:
                console.print(f"[yellow]Tenant already exists: {tenant['display_name']}[/yellow]")

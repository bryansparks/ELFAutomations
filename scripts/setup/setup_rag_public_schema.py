        ...
        if tenants.data:
            ...
            for tenant in tenants.data:
                table.add_row(
                    ...
                    tenant.get("drive_folder_id", "Not Set")[:20] + "..."
                    if tenant.get("drive_folder_id")
                    else "Not Set",
                )
...
def main():
    ...
    if Confirm.ask("\nCreate test tenants?", default=True):
        ...
        try:
            ...
            test_result = (
                client.from_("rag_tenants").select("count", count="exact").execute()
            )
            ...
            if "row-level security" in str(e).lower():
                ...
                console.print(
                    "1. [bold]sql/disable_rag_rls_temporary.sql[/bold] - Temporarily disable RLS"
                )
                console.print(
                    "2. [bold]sql/fix_rag_rls_policies.sql[/bold] - Update RLS policies"
                )

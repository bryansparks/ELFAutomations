...
def check_schema_exists(client: Client) -> bool:
    ...
    try:
        ...
        result = client.schema("rag").from_("tenants").select("*", count="exact").limit(1).execute()
        return True
    except Exception as e:
...
def verify_tables(client: Client) -> dict:
    ...
    tables_to_check = [
        "tenants", "workspaces", "documents", "document_chunks",
        "processing_queue", "entities", "relationships",
        "search_queries", "api_usage", "document_types"
    ]
    ...
    for table in tables_to_check:
        try:
            ...
            result = client.from_(full_table_name).select("*", count="exact").limit(1).execute()
            results[table] = True
        except Exception as e:
            results[table] = False
...
def create_test_tenants(client: Client):
    ...
    for tenant in test_tenants:
        try:
            ...
            result = client.from_("rag.tenants").select("*").eq("name", tenant["name"]).execute()

    ...
    def deploy_qdrant(self) -> bool:
        ...
        try:
            ...
            for i in range(max_attempts):
                result = subprocess.run(
                    ["kubectl", "get", "pod", "-n", "elf-automations", 
                     "-l", "app=qdrant", "-o", "jsonpath={.items[0].status.phase}"],
                    capture_output=True,
                    text=True
                )
        ...
        try:
            ...
            if not db_url:
                ...
                if not db_password:
                    ...
                    logger.info("You can find this in your Supabase project settings under 'Database'")
                    return False
            ...
            try:
                ...
                with conn.cursor() as cur:
                    ...
                    if table_count >= 6:
                        ...
                        logger.error(f"✗ Only found {table_count} memory tables (expected 6+)")
                        all_good = False
...
def main():
    ...
    parser.add_argument(
        "--skip-k8s",
        action="store_true",
        help="Skip Kubernetes deployment (Qdrant)"
    )
    parser.add_argument(
        "--skip-db",
        action="store_true",
        help="Skip database setup (Supabase)"
    )
    parser.add_argument(
        "--verify-only",
        action="store_true",

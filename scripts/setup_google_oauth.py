...
def setup_tenant_oauth(tenant_name: str):
    ...
    try:
        ...
        tenant = supabase.table("rag.tenants").select("*").eq("name", tenant_name).single().execute()
        ...
        console.print("[dim]The browser will redirect to http://localhost:8080 after authorization[/dim]\n")
        ...
        if server.auth_code:
            ...
            if oauth_manager.test_credentials(tenant_id):
                ...
                supabase.table("rag.tenants").update({
                    ...
                    "drive_auth_date": tokens.get("expiry")
...
def list_tenant_auth_status():
    ...
    try:
        ...
        for tenant in tenants.data:
            ...
            status = "[green]✓ Authorized[/green]" if has_oauth else "[red]✗ Not Authorized[/red]"
...
def main():
    ...
    if not os.getenv("GOOGLE_CLIENT_ID") or not os.getenv("GOOGLE_CLIENT_SECRET"):
        ...
        console.print("Please set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET environment variables.\n")

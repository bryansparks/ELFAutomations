...
async def test_full_integration():
    ...
    async with N8NClient() as client:
        ...
        if not workflow:
            ...
            print(
                "   Please run: python scripts/test_n8n_client.py --create-test-workflow"
            )
            return
        ...
        try:
            ...
            print(
                f"  - Duration: {execution.duration:.2f}s" if execution.duration else ""
            )
        ...
        history = await client.get_execution_history(
            workflow_name="test-webhook-workflow", limit=5
        )
        ...
        for exec in history:
            ...
            print(
                f"   {status_icon} {exec.triggered_by} - {exec.started_at.strftime('%Y-%m-%d %H:%M:%S')}"
            )
        ...
        try:
            await client.execute_workflow(
                workflow_name="non-existent-workflow", data={}, team_name="test"
            )
        except Exception as e:
        ...
        if supabase_url and supabase_key:
            ...
            # Get stats from view
            stats = (
                supabase.table("workflow_execution_stats")
                .select("*")

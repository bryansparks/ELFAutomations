...
async def simulate_successful_patterns(team_memory: TeamMemory, agent_role: str):
    ...
    logger.info(
        f"Simulated {sum(p['success_count'] for p in patterns)} successful patterns"
    )
...
async def main():
    ...
    team_memory = TeamMemory(team_name=team_name, collection_name=f"{team_name}_memory")

    learning_system = LearningSystem(team_name=team_name, memory=team_memory)
...
Focus on accuracy and clarity in your analysis.""",
    ...
    evolved_config = loader.load_evolved_agent_config(
        team_id=team_id, agent_role=agent_role, base_config=base_config
    )
    ...
    if evolutions:
        ...
        for i in range(20):
            ...
            ab_testing.record_test_result(
                test_id=test_id, group=group, success=success, duration_seconds=duration
            )
        ...
        print(f"\n   A/B Test Results:")
        print(
            f"   Control (Base) Success Rate: {results.control_metrics['success_rate']:.1%}"
        )
        print(
            f"   Treatment (Evolved) Success Rate: {results.treatment_metrics['success_rate']:.1%}"
        )

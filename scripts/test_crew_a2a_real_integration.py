...
class RealA2AIntegrationTester:
    ...
    async def run_all_tests(self):
        ...
        tests = [
            ("Basic Crew Functionality with Real Services", self.test_basic_crew_functionality),
        ...
        if passed == total:
            logger.info("🎉 All tests passed! CrewAI + Real A2A integration is working correctly.")
        else:
            logger.warning(f"⚠️ {total - passed} tests failed. Check configuration and services.")
    ...
    async def test_basic_crew_functionality(self):
        ...
        assert sales_crew.discovery_service is not None, "Discovery service should be initialized"
    ...
    async def test_real_discovery_service(self):
        ...
        assert wrapper.discovery_service is not None, "Discovery service should be initialized"
        ...
        if wrapper.discovery_service:
            try:
                ...
                # Test discovery query
                agents = await wrapper.discovery_service.discover_agents(["test_capability"])
    ...
    async def test_service_lifecycle(self):
        ...
        for cycle in range(3):
            ...
            assert crew.discovery_service is not None, "Discovery service should be available"
...
async def main():
    ...
    logger.info(f"  Discovery Endpoint: {a2a_config.discovery_endpoint or 'Local registry'}")

...
def test_direct_openrouter():
    ...
    try:
        ...
        console.print(f"[blue]Model used:[/blue] {response.additional_kwargs.get('model_used', 'unknown')}")
        
        # Show metadata
        if "openrouter_metadata" in response.additional_kwargs:
            metadata = response.additional_kwargs["openrouter_metadata"]
            console.print(f"[blue]Routing reason:[/blue] {metadata.get('routing_reason', 'N/A')}")
    ...
    for var in env_vars:
        ...
        if value:
            if "KEY" in var:
                ...
                config_table.add_row(var, "✅ Set", value[:50] + "..." if len(value) > 50 else value)
        else:

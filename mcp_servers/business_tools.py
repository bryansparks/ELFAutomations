...
from .base import BaseMCPServer, MCPTool, MCPResource
...
class BusinessToolsServer(BaseMCPServer):
    ...
    def __init__(self):
        ...
        if supabase_url and supabase_key and SUPABASE_AVAILABLE:
            ...
            self.logger.warning("No database configuration found - some features may be limited")
    ...
    async def _init_supabase(self):
        ...
        try:
            ...
            # Test connection
            result = self.supabase_client.table("customers").select("id").limit(1).execute()
    ...
    async def _execute_query(self, query: str, params: Optional[List] = None) -> List[Dict[str, Any]]:
    ...
    async def _execute_supabase_query(self, query: str, params: Optional[List] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if query_lower.startswith("select"):
                ...
                if "from customers" in query_lower:
                    result = self.supabase_client.table("customers").select("*").execute()
                    return result.data
                    ...
                    result = self.supabase_client.table("business_metrics").select("*").execute()
                    return result.data
                else:
                ...
                self.logger.warning("Non-SELECT queries not yet supported for Supabase", query=query)
                return []
    ...
    async def _execute_postgres_query(self, query: str, params: Optional[List] = None) -> List[Dict[str, Any]]:
    ...
    async def _insert_record(self, table: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    async def _update_record(self, table: str, record_id: int, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Update a record using the appropriate backend."""
        if self.use_supabase and self.supabase_client:
            try:
                result = self.supabase_client.table(table).update(data).eq("id", record_id).execute()
                return result.data[0] if result.data else None
            except Exception as e:
        ...
        self.register_tool(MCPTool(
            ...
            parameters={
                ...
                "properties": {
                    ...
                    "score_change": {"type": "integer", "description": "Score change (+/-)"},
        ...
        self.register_tool(MCPTool(
            ...
            parameters={
                ...
                "properties": {
                    ...
                    "assigned_agent_id": {"type": "string", "description": "Assigned agent filter"},
        ...
        self.register_tool(MCPTool(
            ...
            parameters={
                ...
                "properties": {
                    ...
                    "assigned_agent_id": {"type": "string", "description": "Assigned agent UUID"},
                    "created_by_agent_id": {"type": "string", "description": "Creating agent UUID"},
        ...
        self.register_tool(MCPTool(
            ...
            parameters={
                ...
                "properties": {
                    ...
                    "assigned_agent_id": {"type": "string", "description": "Assigned agent filter"},
                    "created_by_agent_id": {"type": "string", "description": "Creator agent filter"},
        ...
        self.register_tool(MCPTool(
            ...
            parameters={
                ...
                "properties": {
                    ...
                    "time_period": {"type": "string", "description": "Time period (day, week, month)"},
        ...
        if self.use_supabase and self.supabase_client:
            ...
            async with self.db_pool.acquire() as conn:
                await conn.execute("""
                    ...
                    VALUES ($1, $2, $3, $4, $5, $6)
                """, customer_id, args["name"], args["email"], 
                    args.get("phone"), args.get("company"), 
    ...
    async def _get_customer(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.use_supabase and self.supabase_client:
            if "customer_id" in args:
                result = self.supabase_client.table("customers").select("*").eq("id", args["customer_id"]).execute()
                if result.data:
                    return {
                        "success": True,
                ...
                result = self.supabase_client.table("customers").select("*").eq("email", args["email"]).execute()
                if result.data:
                    return {
                        "success": True,
                        "customer": result.data[0]
        ...
        if self.use_supabase and self.supabase_client:
            ...
            async with self.db_pool.acquire() as conn:
                await conn.execute("""
                    ...
                    VALUES ($1, $2, $3, $4, $5, $6)
                """, lead_id, args["customer_id"], args["source"],
                    args.get("score", 0), args.get("assigned_agent_id"),
                    json.dumps(args.get("metadata", {})))
        ...
        if self.use_supabase and self.supabase_client:
            ...
            async with self.db_pool.acquire() as conn:
                result = await conn.execute("""
                    ...
                    WHERE id = $2
                """, args["score_change"], args["lead_id"])
    ...
    async def _create_task(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.use_supabase and self.supabase_client:
            ...
            async with self.db_pool.acquire() as conn:
                ...
                """, task_id, args["title"], args.get("description"),
                    args.get("type"), args.get("priority", 3),
                    args.get("assigned_agent_id"), args["created_by_agent_id"],
                    due_date, json.dumps(args.get("metadata", {})))
        ...
        if self.use_supabase and self.supabase_client:
            ...
            async with self.db_pool.acquire() as conn:
                result = await conn.execute("""
                    ...
                    WHERE id = $3
                """, args["status"], completed_at, args["task_id"])
    ...
    async def _get_tasks(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.use_supabase and self.supabase_client:
            ...
            for field in ["status", "assigned_agent_id", "created_by_agent_id", "priority"]:
                if field in args:
    ...
    async def _get_business_metrics(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.use_supabase and self.supabase_client:
            ...
            if result.data:
                ...
                return {
                    "success": False,
                    "message": "No business metrics found"
                }
        else:
            async with self.db_pool.acquire() as conn:
                customer_count = await conn.fetchval("SELECT COUNT(*) FROM business.customers")
    ...
    async def _read_customers_resource(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "text": json.dumps({
                ...
                "available_tools": ["create_customer", "get_customer", "update_customer"]
            }, indent=2)
        }
    ...
    async def _read_leads_resource(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "text": json.dumps({
                ...
                "available_tools": ["create_lead", "update_lead_score", "get_leads"]
            }, indent=2)
        }
    ...
    async def _read_tasks_resource(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "text": json.dumps({
                ...
                "available_tools": ["create_task", "update_task_status", "get_tasks"]
            }, indent=2)
        }

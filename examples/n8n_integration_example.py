...
async def executive_team_example():
    ...
    async with N8NClient() as client:
        ...
        workflows = await client.list_workflows(
            owner_team="executive-team", category=WorkflowCategory.DATA_PIPELINE
        )
        ...
        history = await client.get_execution_history(
            team_name="executive-team", limit=5
        )

        print("\nRecent workflow executions:")
        for exec in history:
            print(
                f"- {exec.workflow_name}: {exec.status.value} "
                f"(took {exec.duration:.2f}s)"
                if exec.duration
                else ""
            )
    ...
    class WorkflowAgent(Agent):
        ...
        async def execute_workflow(self, workflow_name: str, data: dict):
            ...
            async with self.n8n_client as client:
                result = await client.execute_workflow(
                    workflow_name=workflow_name, data=data, team_name=self.role
                )

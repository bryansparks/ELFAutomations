...
async def demo_workflow_creation():
    ...
    print("Node Types:", [node['type'].split('.')[-1] for node in sales_workflow['workflow']['nodes']])
    ...
    for result in batch_results:
        print(f"  - {result['workflow']['name']} ({result['metadata']['trigger_type']})")
...
def show_statistics():
    ...
    print(f"⏱️  If each saves 2 hours/week: {int(total_workflows * 0.7 * 2)} hours saved weekly")

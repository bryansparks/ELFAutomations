...
class IntegrationTest:
    ...
    async def test_prerequisites(self):
        ...
        try:
            result = subprocess.run(
                ["kubectl", "get", "crd", "agents.kagent.io"],
                capture_output=True,
                text=True,
            )
            if result.returncode != 0:
    ...
    def start_agentgateway(self):
        ...
        try:
            ...
            subprocess.run(
                ["docker", "stop", self.agentgateway_container],
                capture_output=True,
                text=True,
            )
            subprocess.run(
                ["docker", "rm", self.agentgateway_container],
                capture_output=True,
                text=True,
            )
            ...
            result = subprocess.run(
                [
                    ...
                    "-d",
                    "--name",
                    self.agentgateway_container,
                    "-p",
                    f"{self.agentgateway_port}:3000",
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if result.returncode == 0:
                    ...
                    logger.info(
                        f"   Department: {agent_data['spec'].get('department', 'Unknown')}"
                    )
                    logger.info(
                        f"   Replicas: {agent_data['spec'].get('replicas', 'Unknown')}"
                    )
                    return True
                else:
    ...
    async def test_distributed_agent_creation(self):
        ...
        try:
            ...
            result = subprocess.run(
                [
                    "python",
                    "/Users/bryansparks/projects/ELFAutomations/test_distributed_standalone.py",
                ],
                capture_output=True,
                text=True,
                cwd="/Users/bryansparks/projects/ELFAutomations",
            )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                response = await client.get(
                    f"http://localhost:{self.agentgateway_port}/", timeout=5.0
                )
                logger.info(
                    f"✅ AgentGateway responding (status: {response.status_code})"
                )
    ...
    async def run_all_tests(self):
        ...
        try:
            ...
            if agentgateway_started:
                results[
                    "agentgateway_connectivity"
                ] = await self.test_agentgateway_connectivity()
            else:

...
async def test_kagent_integration():
    ...
    if not check_kagent_controller():
        logger.warning("⚠️  kagent controller not found. This is expected if not deployed yet.")
    else:
    ...
    try:
        result = subprocess.run([
            'kubectl', 'get', 'events', '-n', 'elf-automations', 
...
async def test_docker_build():
    ...
    dockerfile_path = "/Users/bryansparks/projects/ELFAutomations/agents/distributed/Dockerfile"
    ...
    try:
        ...
        result = subprocess.run([
            'docker', 'build', 
            '-t', 'elf-automations/sales-agent:latest',
            '-f', dockerfile_path,

...
class K8sFoundationTestSuite(TestSuite):
    ...
    async def test_crd_installation(self):
        ...
        success, stdout, stderr = await run_command(["kubectl", "get", "crd"])
        if not success:
        ...
        for crd in expected_crds:
            if crd not in stdout:
                ...
                if "kagent" not in stdout:
                    raise Exception(
                        f"kagent CRDs not installed. Expected: {expected_crds}"
                    )
    ...
    async def test_resource_quotas(self):
        ...
        success, stdout, stderr = await run_command(["kubectl", "top", "nodes"])
        ...
        success, stdout, stderr = await run_command(["kubectl", "describe", "nodes"])
        if not success:

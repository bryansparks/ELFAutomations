        ...
        for image in required_images:
            ...
            if not success or image.split(':')[0] not in stdout:
                ...
                if not dockerfile_found:
                    raise Exception(f"Image {image} not available and no Dockerfile found")
...
"""
    ...
    async def test_agent_pod_health(self):
        ...
        async def pod_running():
            success, stdout, stderr = await run_command([
                ...
                '-l', 'app=chief-ai-agent-test', '-o', 'jsonpath={.items[0].status.phase}'
            ])
        ...
        if not await wait_for_condition(pod_running, timeout=120):
            ...
            success, stdout, stderr = await run_command([
                'kubectl', 'describe', 'pods', '-n', self.test_namespace,
                '-l', 'app=chief-ai-agent-test'
            ])
    ...
    async def test_agent_api_endpoints(self):
        ...
        port_forward_process = await asyncio.create_subprocess_exec(
            ...
            f'service/chief-ai-agent-test-service', '18091:8091',
            '-n', self.test_namespace,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        ...
        try:
            ...
            success, message = await check_http_endpoint('http://localhost:18091/health')
            if not success:
    ...
    async def test_crewai_task_execution(self):
        ...
        success, stdout, stderr = await run_command([
            ...
            '-l', 'app=chief-ai-agent-test', '--tail=50'
        ])
        ...
        found_indicators = [indicator for indicator in crewai_indicators 
                          if indicator.lower() in log_content]
        
        if len(found_indicators) < 2:
            raise Exception(f"CrewAI activity not detected in logs. Found: {found_indicators}")
            
    async def test_agent_scaling(self):
        """Test agent scaling capabilities."""
        # Scale up the deployment
        success, stdout, stderr = await run_command([
            'kubectl', 'scale', 'deployment', 'chief-ai-agent-test',
            '--replicas=2', '-n', self.test_namespace
        ])
        ...
        async def scaled_up():
            success, stdout, stderr = await run_command([
                'kubectl', 'get', 'deployment', 'chief-ai-agent-test',
                '-n', self.test_namespace, '-o', 'jsonpath={.status.readyReplicas}'
            ])
        ...
        success, stdout, stderr = await run_command([
            'kubectl', 'scale', 'deployment', 'chief-ai-agent-test',
            '--replicas=1', '-n', self.test_namespace
        ])
    ...
    async def test_agent_lifecycle_management(self):
        ...
        success, stdout, stderr = await run_command([
            'kubectl', 'patch', 'deployment', 'chief-ai-agent-test',
            '-n', self.test_namespace,
            '-p', '{"spec":{"template":{"metadata":{"labels":{"version":"test-update"}}}}}'
        ])
        ...
        success, stdout, stderr = await run_command([
            'kubectl', 'rollout', 'status', 'deployment/chief-ai-agent-test',

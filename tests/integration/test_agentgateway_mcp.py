...
class AgentGatewayMCPTestSuite(TestSuite):
    ...
    async def test_agentgateway_configuration(self):
        ...
        test_config = {
            "type": "static",
            "listeners": [
                {"protocol": "MCP", "transport": "stdio", "address": "0.0.0.0:3000"}
            ],
            "targets": {"mcp": []},
        }
        ...
        success, stdout, stderr = await run_command(
            [
                ...
                "--rm",
                "-v",
                "/tmp/agentgateway-test-config.json:/config.json",
                "elf-automations/agentgateway:latest",
                "--file",
                "/config.json",
                "--validate",
...
"""
    ...
    async def test_agentgateway_health(self):
        ...
        port_forward_process = await asyncio.create_subprocess_exec(
            "kubectl",
            "port-forward",
            f"service/agentgateway-test-service",
        ...
        try:
            ...
            if not success:
                raise Exception(
                    f"AgentGateway port {self.agentgateway_port} not accessible"
                )
...
"""
        ...
        success, stdout, stderr = await run_command(["python3", "-c", test_script])
    ...
    async def test_tool_execution_via_gateway(self):
        ...
        mcp_port_forward = await asyncio.create_subprocess_exec(
            "kubectl",
            "port-forward",
            "service/test-mcp-server-service",
            "18080:8080",
            "-n",
            self.test_namespace,
            stdout=asyncio.subprocess.PIPE,
...
"""
        ...
        if not success:
            raise Exception(
                f"Failed to update AgentGateway for multiple servers: {stderr}"
            )
    ...
    async def test_gateway_error_handling(self):
        ...
        if not success:
            raise Exception(
                f"Failed to update AgentGateway for error testing: {stderr}"
            )
        ...
        if not success or stdout.strip() != "Running":
            raise Exception(
                "AgentGateway not handling unavailable MCP server gracefully"
            )

...
class TestLangGraphAgentGatewayIntegration:
    ...
    async def test_agent_tool_discovery(self, test_agent):
        ...
        mock_tools = [
            ...
            {
                "name": "business/get_metrics",
                "description": "Get business metrics"
            }
        ]
        
        with patch.object(test_agent.gateway_client, 'list_available_tools', return_value=mock_tools):
            await test_agent.start()
            
            # Check that tools were discovered
            assert len(test_agent.available_tools) == 2
            assert any(tool["name"] == "supabase/list_tables" for tool in test_agent.available_tools)
            assert any(tool["name"] == "business/get_metrics" for tool in test_agent.available_tools)
    ...
    async def test_agent_tool_execution(self, test_agent):
        ...
        mock_result = {
            "content": [
                {
                    "type": "text", 
                    "text": "Database tables: customers, leads, tasks"
                }
            ]
        }
        
        with patch.object(test_agent.gateway_client, 'call_mcp_tool', return_value=mock_result):
            await test_agent.start()
...
class TestAgentGatewayDeployment:
    ...
    def test_kubernetes_manifests_exist(self):
        ...
        base_path = os.path.join(os.path.dirname(__file__), '..', 'k8s', 'base', 'agentgateway')
...
class TestMCPProtocolCompliance:
    ...
    def test_tool_name_format(self):
        ...
        expected_business_name = f"{business_tool.server_name}/{business_tool.tool_name}"
...
async def run_integration_tests():
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get("http://localhost:8080/health") as response:
                if response.status == 200:
                    ...
                    print(f"   ❌ AgentGateway admin endpoint returned {response.status}")
    ...
    try:
        gateway_client = AgentGatewayClient(
            gateway_url="http://localhost:3000",
            agent_id="federation-test"
        )
        
        async with gateway_client as client:
            tools = await client.list_available_tools()
            
            supabase_tools = [t for t in tools if t.get('name', '').startswith('supabase/')]
            business_tools = [t for t in tools if t.get('name', '').startswith('business/')]

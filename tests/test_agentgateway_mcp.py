...
def start_agentgateway():
    ...
    try:
        ...
        subprocess.run(
            ["docker", "stop", "agentgateway-test"], capture_output=True, text=True
        )
        subprocess.run(
            ["docker", "rm", "agentgateway-test"], capture_output=True, text=True
        )
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                "http://localhost:3002/mcp/servers", timeout=10.0
            )
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"⚠️  MCP servers endpoint returned: {response.status_code}"
                )
...
async def test_agentgateway_logs():
    ...
    try:
        result = subprocess.run(
            ["docker", "logs", "agentgateway-test"], capture_output=True, text=True
        )
...
async def test_basic_connectivity():
    ...
    try:
        ...
        result = subprocess.run(
            ["docker", "exec", "agentgateway-test", "nc", "-z", "localhost", "3000"],
            capture_output=True,
            text=True,
        )
        ...
        # Test from host
        result = subprocess.run(
            ["nc", "-z", "localhost", "3002"], capture_output=True, text=True
        )

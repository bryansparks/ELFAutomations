...
def trigger_n8n_workflow(
    ...
    try:
        ...
        return {
            ...
            if response.text
            else {"message": "Workflow triggered"},
        }
...
def check_workflow_execution_status(execution_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if result.data:
            return {"success": True, "execution": result.data[0]}
        else:
...
def list_available_workflows() -> List[Dict[str, Any]]:
    ...
    try:
        supabase = get_supabase_client()
        result = (
            supabase.table("workflow_registry").select("*").eq("active", True).execute()
        )
...
def validate_workflow_input(
    ...
    try:
        ...
        if not result.data:
            return {"valid": False, "errors": ["Workflow not found in registry"]}

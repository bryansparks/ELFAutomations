"""
Validation Specialist Agent for n8n-interface-team
"""
...
def request_validator(
    ...
    """
    Validation Specialist implementation
    ...
    Skills: schema validation, data validation, error reporting, API specifications

    Personality Traits: detail-oriented
    """
    # Create the agent using factory function
    if not llm:
        llm = LLMFactory.create_llm(
            provider="openai", model="gpt-4", enable_fallback=True
        )

    # Get the enhanced system prompt with personality traits
    backstory = """You are the Validation Specialist for n8n-interface-team.
    ...
    if enable_evolution:
        try:
            ...
            base_config = {
                ...
                "goal": "Validates incoming workflow requests against schema definitions. Ensures all required parameters are present and properly formatted",
                "backstory": backstory,
            ...
            if evolved_config and evolved_config.evolved_prompt:
                ...
                logger.info(
                    f"Loaded evolved prompt for Validation Specialist (confidence: {evolved_config.confidence_score})"
                )

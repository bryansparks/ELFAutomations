...
class A2AClient:
    ...
    def __init__(
        ...
        self.gateway_url = gateway_url or os.getenv(
            "A2A_GATEWAY_URL", "http://a2a-gateway-service:8080"
        )
    ...
    async def send_task(
        ...
        logger.info(
            f"A2A Direct: {self.team_id} -> {target_team}: {task_description[:50]}..."
        )
    ...
    async def _send_task_via_gateway(
        ...
        logger.info(
            f"A2A Gateway: {self.team_id} routing task: {task_description[:50]}..."
        )
        ...
        try:
            response = await self.client.post(
                f"{self.gateway_url}/route", json=payload, timeout=timeout
            )
            response.raise_for_status()
    ...
    async def discover_teams(
        ...
        try:
            ...
            response = await self.client.get(f"{self.gateway_url}/teams", params=params)
            response.raise_for_status()

...
class ProjectAwareMixin(ABC):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.project_logger = logging.getLogger(
            f"{self.__class__.__name__}.ProjectAware"
        )
    ...
    async def find_and_claim_work(self) -> bool:
        ...
        try:
            ...
            claim_result = await self.project_mcp.call_tool(
                "assign_task",
                {"task_id": best_task["id"], "team_id": self.get_team_id()},
            )
    ...
    async def _execute_task(self, task: Dict):
        ...
        try:
            ...
            await self.report_task_blocker(
                f"Encountered error: {str(e)}", severity="high"
            )
    ...
    async def update_task_progress(
        ...
        try:
            ...
            result = await self.project_mcp.call_tool("update_task_status", update_data)

            if result.get("success"):
                # Send progress update to project owner
                if self.current_task.get("project_id"):
                    await self._send_progress_update(
                        status, progress, hours_worked, notes
                    )
        ...
        try:
            ...
            if result.get("success"):
                ...
                await self._send_blocker_notification(
                    blocker_description, severity, needs_help_from
                )
                return True
    ...
    async def _send_blocker_notification(
        self, blocker_description: str, severity: str, needs_help_from: Optional[str]
    ):

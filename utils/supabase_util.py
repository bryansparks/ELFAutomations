    ...
    try:
        ...
        for table in tables_to_check:
            try:
                ...
                logger.warning(
                    f"Table {table} does not exist or is not accessible", error=str(e)
                )
        ...
        if len(existing_tables) == len(tables_to_check):
            ...
            logger.warning(
                f"Only {len(existing_tables)}/{len(tables_to_check)} tables are accessible"
            )
            logger.info(
                "You may need to create tables manually in your Supabase dashboard"
            )
            return False
...
async def insert_sample_data():
    ...
    try:
        ...
        leads_data = [
            ...
            {
                "name": "Charlie Wilson",
                "email": "charlie@potential.org",
                "company": "Potential Org",
                "source": "referral",
                "status": "new",
                "score": 65,
            },
            {
                "name": "Diana Davis",
                "email": "diana@opportunity.net",
                "company": "Opportunity Net",
                "source": "social",
                "status": "contacted",
                "score": 75,
            },
        ...
        tasks_data = [
            ...
            },
            {
                ...
                "description": "Compile quarterly business metrics",
                "assignee": "analytics-agent-001",
                "status": "in_progress",
                "priority": "medium",
            },
            {
        ...
        for table_name, data in datasets:
            try:
                ...
                logger.warning(
                    f"Could not insert sample data into {table_name}", error=str(e)
                )

        logger.info(
            f"Sample data insertion completed: {success_count}/{len(datasets)} tables"
        )
...
def query_table(
    table_name: str, filters: Optional[Dict[str, Any]] = None, limit: int = 10
) -> List[Dict[str, Any]]:
...
def update_record(
    table_name: str, record_id: Any, data: Dict[str, Any]

...
class MinIOTenantManager:
    ...
    def __init__(
        ...
        self.secret_key = secret_key or os.getenv("MINIO_SECRET_KEY", "elfautomations2025secure")
        self.secure = secure
    ...
    def store_document(
        ...
        try:
            ...
            logger.info(f"Stored document {document_id} for tenant {tenant_id}: {object_key}")
    ...
    def delete_document(
        ...
        for doc in documents:
            if doc.get("document_id") == document_id and doc.get("filename") == filename:
                object_key = doc["key"]
                break
    ...
    def create_presigned_url(
        ...
        if not object_key:
            logger.warning(f"Document not found for presigned URL: {document_id}/{filename}")
            return None
    ...
    def setup_bucket_notifications(
        ...
        logger.info(f"Notification configuration for {bucket_name} (enterprise feature)")
        return True

...
class MockQdrantClient:
            ...
            "config": type('obj', (object,), {
                "params": type('obj', (object,), {
                    "vectors": type('obj', (object,), {
                        "size": collection["config"].size if collection["config"] else 384,
                        "distance": "Cosine"
                    })
                })
            })
    ...
    def upsert(self, collection_name: str, points: List[Any]):
        ...
        logger.info(f"[MOCK QDRANT] Upserted {len(points)} points into {collection_name}")
    ...
    def search(
        ...
        logger.info(f"[MOCK QDRANT] Searched {collection_name}, returning {len(results)} results")
        return results
    ...
    def _apply_filter(self, points: List[MockPoint], query_filter: Any) -> List[MockPoint]:
            ...
            if hasattr(query_filter, 'must') and query_filter.must:
                for condition in query_filter.must:
                    if hasattr(condition, 'key') and hasattr(condition, 'match'):
                        ...
                        if key not in point.payload or point.payload[key] != expected_value:
                            include = False
                            break

...
class MemoryAgentMixin:
    ...
    def init_memory(self, team_name: str, qdrant_url: str = "http://localhost:6333"):
        ...
        self.memory_logger = logging.getLogger(
            f"MemoryAgent.{team_name}.{getattr(self, 'role', 'agent')}"
        )
    ...
    def complete_episode(
        self, success: bool, result: Any = None, error: Optional[Dict[str, Any]] = None
    ):
        """
    ...
    def _recall_relevant_experiences(self, task_description: str):
        ...
        similar_episodes = self.team_memory.recall_similar_episodes(
            query_embedding, limit=3
        )

        if similar_episodes:
            self.memory_logger.info(
                f"Recalled {len(similar_episodes)} similar past episodes"
            )
    ...
    def share_knowledge(
        self, insight: str, context: Dict[str, Any], confidence: float = 0.8
    ):
        """

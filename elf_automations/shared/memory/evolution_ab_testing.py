...
class EvolutionABTesting:
    ...
    def create_ab_test(
        ...
        try:
            ...
            test_data = {
                ...
                "end_time": (
                    datetime.utcnow() + timedelta(hours=test_duration_hours)
                ).isoformat(),
    ...
    def record_test_result(
        ...
        try:
            ...
            self.supabase.table("ab_tests").update(
                {"metrics": metrics, "updated_at": datetime.utcnow().isoformat()}
    ...
    def get_test_results(self, test_id: str) -> ABTestResult:
        ...
        try:
            # Get test data
            result = (
                self.supabase.table("ab_tests").select("*").eq("id", test_id).execute()
            )
    ...
    def _calculate_confidence(self, metrics: Dict) -> float:
        ...
        total_samples = (
            metrics["control"]["requests"] + metrics["treatment"]["requests"]
        )

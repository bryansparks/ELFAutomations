...
class PromptEvolution:
    ...
    def get_evolved_prompt(
        ...
        try:
            ...
            result = self.supabase.table('agent_evolutions').select(
                'evolved_version, confidence_score'
            ).eq(
                'team_id', team_id
            ).eq(
            ...
            if result.data:
                ...
                if evolution['confidence_score'] >= 0.9:
                    logger.info(f"Using evolved prompt for {agent_role} (confidence: {evolution['confidence_score']})")
    ...
    def _get_proven_strategies(
        ...
        try:
            ...
            query = self.supabase.table('team_learnings').select(
                'pattern, success_rate, usage_count, context'
            ).eq(
                'team_id', team_id
            ).gte(
    ...
    def _store_evolution(
        ...
        try:
            ...
            logger.info(f"Stored prompt evolution for {agent_role} (confidence: {confidence:.2f})")
                ...
                if 'review_improves_quality' in pattern:
                    if 'review' not in evolved_workflow.get('nodes', []):
                        evolved_workflow.setdefault('nodes', []).append({
                            ...
                            'type': 'review',
                            'config': {
                                'criteria': learning.get('context', {}).get('criteria', [])
                            }
                        })
    ...
    def get_evolution_history(
        ...
        try:
            query = self.supabase.table('agent_evolutions').select('*').eq(
                'team_id', team_id
            )
    ...
    def rollback_evolution(self, evolution_id: str) -> bool:
        ...
        try:
            ...
            self.supabase.table('agent_evolutions').insert({
                ...
                'original_version': evolution['evolved_version'],  # Current becomes original
                'evolved_version': evolution['original_version'],  # Revert to original

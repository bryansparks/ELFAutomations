...
class LearningSystem:
    ...
    def _identify_success_factors(self, episode: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if duration > 0:
            ...
            similar_episodes = self._get_similar_past_episodes(episode['task_description'])
            if similar_episodes:
                avg_duration = np.mean([e['duration_seconds'] for e in similar_episodes])
    ...
    def _identify_failure_causes(self, episode: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        contributions = episode.get('agent_contributions', {})
        expected_agents = self._get_expected_agents_for_task(episode['task_description'])
    ...
    def synthesize_strategy(self, task_type: str) -> Optional[Dict[str, Any]]:
        ...
        if len(patterns) < self.min_episodes_for_learning:
            self.logger.info(f"Insufficient data for strategy synthesis ({len(patterns)} patterns)")
            return None
        ...
        if recent_episodes:
            durations = [e['duration_seconds'] for e in recent_episodes if e.get('duration_seconds')]
            if durations:
        ...
        self.logger.info(f"Synthesized strategy for {task_type} with confidence {strategy['confidence']}")
        return strategy
    
    def predict_success_probability(self, task_description: str, 
                                   planned_approach: Dict[str, Any]) -> float:
        """
        ...
        base_success_rate = sum(1 for e in recent_episodes if e['success']) / len(recent_episodes)
    ...
    def recommend_improvements(self, recent_episodes: int = 10) -> List[Dict[str, Any]]:
        ...
        if inactive_agents:
            recommendations.append({
                ...
                'evidence': {
                    'inactive_agents': list(inactive_agents)
                }
            })
    ...
    def _get_recent_episodes(self, task_type: str, days: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Get team_id
            team_result = self.memory.supabase.table('teams').select('id').eq('name', self.team_name).execute()
            team_id = team_result.data[0]['id'] if team_result.data else None
            
            if not team_id:
                return []
            
            result = self.memory.supabase.table('memory_entries').select('*').eq(
                'team_id', team_id
            ...
            for entry in result.data:
                ...
                if self._categorize_task(content.get('task_description', '')) == task_type:
                    episodes.append({
                        'id': entry['id'],
    ...
    def _get_recent_successful_episodes(self, task_type: str, days: int) -> List[Dict[str, Any]]:

...
from .models import (
    WorkflowSpec, WorkflowExecution, WorkflowInfo,
    WorkflowStatus, WorkflowTriggerType, WorkflowCategory
)
...
class N8NClient:
    ...
    def __init__(self, 
        ...
        """
        # N8N configuration
        self.n8n_url = n8n_url or os.getenv('N8N_URL', 'http://n8n.n8n.svc.cluster.local:5678')
        self.n8n_api_key = n8n_api_key or os.getenv('N8N_API_KEY', '')
        
        # Supabase configuration
        supabase_url = supabase_url or os.getenv('SUPABASE_URL')
        supabase_key = supabase_key or os.getenv('SUPABASE_KEY') or os.getenv('SUPABASE_ANON_KEY') or os.getenv('SUPABASE_SECRET_KEY')
        
        if not supabase_url or not supabase_key:
            raise N8NError("Supabase credentials not provided. Set SUPABASE_URL and SUPABASE_KEY (or SUPABASE_ANON_KEY) in environment")
    ...
    async def execute_workflow(self, 
                             ...
                             timeout: int = 300) -> WorkflowExecution:
        """
    ...
    async def get_workflow_info(self, workflow_name: str) -> Optional[WorkflowInfo]:
        ...
        try:
            response = self.supabase.table('n8n_workflows').select('*').eq(
                'name', workflow_name
            ).single().execute()
    ...
    async def list_workflows(self, 
                           ...
                           active_only: bool = True) -> List[WorkflowInfo]:
    ...
    async def get_execution_history(self,
        ...
        workflow_names = {}
        if response.data:
            workflow_ids = list(set(data['workflow_id'] for data in response.data if data['workflow_id']))
            if workflow_ids:
                workflows = self.supabase.table('n8n_workflows').select('id, name').in_('id', workflow_ids).execute()
    ...
    async def _execute_webhook(self, 
                             webhook_url: str, 
                             data: Dict[str, Any],
        ...
        async with self.session.post(
            ...
            if response.status != 200:
                ...
                raise WorkflowExecutionError("webhook", f"HTTP {response.status}: {text}")
    ...
    async def _update_execution_record(self,
                                     execution_id: str,
                                     status: WorkflowStatus,
                                     output_data: Optional[Dict[str, Any]] = None,
                                     error_message: Optional[str] = None) -> WorkflowExecution:
        ...
        response = self.supabase.table('workflow_executions').update(
            update_data
        ...
        # Get workflow name
        workflow_result = self.supabase.table('n8n_workflows').select('name').eq('id', data['workflow_id']).single().execute()
        workflow_name = workflow_result.data['name'] if workflow_result.data else 'Unknown'

...
class TeamRegistryClient:
    ...
    async def get_team_hierarchy(self, team_name: str) -> Optional[TeamHierarchy]:
        ...
        try:
            ...
            if result.get("success"):
                ...
                # Build team objects
                team = await self._build_team_with_members(hierarchy_data.get("team", {}))
                parent = None
                if hierarchy_data.get("parent"):
                    parent = await self._build_team_with_members(hierarchy_data["parent"])
    ...
    async def update_team_capabilities(self, team_name: str, agent_name: str, capabilities: List[str]) -> bool:
    ...
    def register_team_sync(self, name: str, type: str, parent_name: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> Team:

-- Multi-Tenant RAG System Database Schema (Public Schema Version)
-- This version creates all tables in the public schema for easier API access
...
CREATE TABLE IF NOT EXISTS rag_document_types (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name TEXT UNIQUE NOT NULL,
    display_name TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS rag_documents (
    ...
    document_type_id UUID REFERENCES rag_document_types(id),
...
CREATE TABLE IF NOT EXISTS rag_search_queries (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL REFERENCES rag_tenants(id) ON DELETE CASCADE,
    workspace_id UUID REFERENCES rag_workspaces(id),
...
-- API usage tracking
CREATE TABLE IF NOT EXISTS rag_api_usage (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL REFERENCES rag_tenants(id) ON DELETE CASCADE,
...
-- OAuth events tracking
CREATE TABLE IF NOT EXISTS rag_oauth_events (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL REFERENCES rag_tenants(id) ON DELETE CASCADE,
...
-- Documents indexes
CREATE INDEX IF NOT EXISTS idx_rag_documents_tenant_status ON rag_documents(tenant_id, status);
CREATE INDEX IF NOT EXISTS idx_rag_documents_source ON rag_documents(source_type, source_id);
CREATE INDEX IF NOT EXISTS idx_rag_documents_created ON rag_documents(tenant_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_rag_documents_type ON rag_documents(document_type_id);
...
-- Queue indexes
CREATE INDEX IF NOT EXISTS idx_rag_queue_status_priority ON rag_processing_queue(status, priority DESC);
CREATE INDEX IF NOT EXISTS idx_rag_queue_tenant ON rag_processing_queue(tenant_id, status);

-- Entities indexes
CREATE INDEX IF NOT EXISTS idx_rag_entities_tenant_type ON rag_entities(tenant_id, entity_type);
CREATE INDEX IF NOT EXISTS idx_rag_entities_normalized ON rag_entities(normalized_name);

-- Search indexes
CREATE INDEX IF NOT EXISTS idx_rag_search_tenant_created ON rag_search_queries(tenant_id, created_at DESC);

-- API usage indexes
CREATE INDEX IF NOT EXISTS idx_rag_api_usage_tenant_created ON rag_api_usage(tenant_id, created_at DESC);
...
CREATE POLICY rag_tenant_isolation ON rag_tenants
    FOR ALL
    USING (id = current_setting('app.current_tenant_id', true)::uuid);

-- Workspaces: Only see workspaces in your tenant
CREATE POLICY rag_workspace_isolation ON rag_workspaces
    FOR ALL
...
CREATE POLICY rag_chunk_isolation ON rag_document_chunks
    FOR ALL
    USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);

-- Queue: Only see your tenant's queue items
CREATE POLICY rag_queue_isolation ON rag_processing_queue
    FOR ALL
    USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);

-- Entities: Only see your tenant's entities
CREATE POLICY rag_entity_isolation ON rag_entities
    FOR ALL
    USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);

-- Relationships: Only see your tenant's relationships
CREATE POLICY rag_relationship_isolation ON rag_relationships
    FOR ALL
    USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);

-- Search queries: Only see your tenant's queries
CREATE POLICY rag_search_isolation ON rag_search_queries
    FOR ALL
    USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);

-- API usage: Only see your tenant's usage
CREATE POLICY rag_api_usage_isolation ON rag_api_usage
    FOR ALL
    USING (tenant_id = current_setting('app.current_tenant_id', true)::uuid);
...
CREATE OR REPLACE FUNCTION rag_set_tenant_context(tenant_id UUID)
RETURNS void AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION rag_get_current_tenant()
RETURNS UUID AS $$
BEGIN
...
CREATE TRIGGER update_rag_tenants_updated_at BEFORE UPDATE ON rag_tenants
    FOR EACH ROW EXECUTE FUNCTION rag_update_updated_at_column();

CREATE TRIGGER update_rag_workspaces_updated_at BEFORE UPDATE ON rag_workspaces
    FOR EACH ROW EXECUTE FUNCTION rag_update_updated_at_column();
...
INSERT INTO rag_document_types (name, display_name, processor_config, storage_strategy) VALUES
('invoice', 'Invoice', 
...
INSERT INTO rag_tenants (name, display_name, settings) VALUES
...
CREATE OR REPLACE VIEW rag_document_status AS
SELECT 
...
JOIN rag_tenants t ON d.tenant_id = t.id
LEFT JOIN rag_workspaces w ON d.workspace_id = w.id
LEFT JOIN rag_document_types dt ON d.document_type_id = dt.id;

-- Tenant usage summary
CREATE OR REPLACE VIEW rag_tenant_usage AS
SELECT 
    t.id as tenant_id,
    t.name as tenant_name,
...
FROM rag_tenants t
LEFT JOIN rag_documents d ON t.id = d.tenant_id
LEFT JOIN rag_search_queries sq ON t.id = sq.tenant_id
LEFT JOIN rag_api_usage au ON t.id = au.tenant_id
GROUP BY t.id, t.name;

-- Processing queue status
CREATE OR REPLACE VIEW rag_queue_status AS
SELECT 
    t.name as tenant_name,
    pq.status,
    COUNT(*) as count,
...
FROM rag_processing_queue pq
JOIN rag_tenants t ON pq.tenant_id = t.id
...
COMMENT ON TABLE rag_tenants IS 'Organizations using the RAG system';
COMMENT ON TABLE rag_documents IS 'Master record for all ingested documents with multi-tenant isolation';
COMMENT ON TABLE rag_processing_queue IS 'Async processing queue for document ingestion';
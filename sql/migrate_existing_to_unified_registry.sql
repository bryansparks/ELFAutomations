-- Migration to Unified Workflow Registry
-- Handles existing tables and data safely

-- First, check what tables exist and rename/migrate them
DO $$
DECLARE
    table_exists BOOLEAN;
BEGIN
    -- Check if old workflow_registry table exists
    SELECT EXISTS (
    ...
    IF table_exists THEN
        RAISE NOTICE 'Found existing workflow_registry table, renaming to backup...';
        ALTER TABLE workflow_registry RENAME TO workflow_registry_backup;
    END IF;

    -- Check if old n8n_workflow_registry exists
    SELECT EXISTS (
        SELECT FROM information_schema.tables
        WHERE table_name = 'n8n_workflow_registry'
    ) INTO table_exists;

    IF table_exists THEN
        RAISE NOTICE 'Found existing n8n_workflow_registry table, renaming to backup...';
        ALTER TABLE n8n_workflow_registry RENAME TO n8n_workflow_registry_backup;
    END IF;
END $$;

-- Drop any existing views that might conflict
DROP VIEW IF EXISTS active_workflows CASCADE;
DROP VIEW IF EXISTS workflow_templates_view CASCADE;

-- Now create the new unified schema
...
DROP TRIGGER IF EXISTS set_workflow_version ON workflow_versions;
...
DECLARE
    ...
    rec RECORD;
BEGIN
    -- Migrate from workflow_registry_backup if it exists
    SELECT EXISTS (
        SELECT FROM information_schema.tables
        WHERE table_name = 'workflow_registry_backup'
    ) INTO table_exists;

    IF table_exists THEN
        RAISE NOTICE 'Migrating data from workflow_registry_backup...';

        FOR rec IN SELECT * FROM workflow_registry_backup LOOP
            INSERT INTO workflows (
                ...
                COALESCE(rec.id::text, gen_random_uuid()::text),
                rec.name,
                rec.description,
                rec.category,
                rec.owner_team,
        ...
        RAISE NOTICE 'Migration from workflow_registry_backup completed.';
    END IF;

    -- Migrate from n8n_workflow_registry_backup if it exists
    SELECT EXISTS (
        SELECT FROM information_schema.tables
        WHERE table_name = 'n8n_workflow_registry_backup'
    ) INTO table_exists;

    IF table_exists THEN
        RAISE NOTICE 'Migrating data from n8n_workflow_registry_backup...';

        FOR rec IN SELECT * FROM n8n_workflow_registry_backup LOOP
            INSERT INTO workflows (
                workflow_id,
                name,
                description,
        ...
        RAISE NOTICE 'Migration from n8n_workflow_registry_backup completed.';
    END IF;
END $$;
...
DO $$
BEGIN
    RAISE NOTICE 'Migration to unified workflow registry completed successfully!';
END $$;

...
from agents.distributed.base.distributed_agent import DistributedCrewAIAgent, AgentConfig
from agents.distributed.a2a.messages import create_message, MessageType
...
async def create_distributed_agents():
    ...
    chief_config = AgentConfig(
        ...
        a2a_host="0.0.0.0",
        a2a_port=8090,
        discovery_endpoint=os.getenv("DISCOVERY_ENDPOINT", "http://discovery-service:8080")
    )
...
async def test_a2a_communication(agents):
    ...
    try:
        ...
        if hasattr(chief_agent, 'a2a_client_manager') and chief_agent.a2a_client_manager:
            result = await chief_agent.a2a_client_manager.send_message(sales_agent.config.agent_id, message)
...
async def coordinate_business_operations(agents):
    ...
    marketing_agent = next((a for a in agents if "marketing" in a.config.agent_id), None)
    ...
    crew = Crew(
        agents=[chief_agent.crew_agent, sales_agent.crew_agent, marketing_agent.crew_agent],
